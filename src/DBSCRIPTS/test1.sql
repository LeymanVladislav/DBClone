declare
  ...
  v$EnableTCT     boolean := case ? when 1 then true else false end; --true; --Включает создание констреинтов для таблиц
  v$EnableIDX     boolean := case ? when 1 then true else false end; --true; --Включает создание индексов для таблиц
  v$DisablePK4IDX boolean := true; --Отключает отдельное создание индекса для ПК
  ...
  begin
    ...
    loop
      ...
      for i in 1 .. cardinality(v_obj$ddls) loop
        -- Отключение создания индекса для первичного ключа
